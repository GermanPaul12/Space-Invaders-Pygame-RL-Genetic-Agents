...
# This should be defined in a central place like utils/constants.py or passed around
# For now, let's assume it's available or we define it here for CLI help text.
...
def add_agent_config_path_args(parser):
    ...
    for agent_type in TRAINABLE_AGENTS_DEFAULT_LIST: # Only for trainable agents
...
def add_gif_args(parser, default_max_frames=600): # MAX_FRAMES_PER_GIF_SEGMENT default
...
def add_common_eval_args(parser):
    ...
    parser.add_argument("--max_steps_per_episode", type=int, default=3000)
    # Evaluation is typically headless and silent for speed and consistency.
    # These can be hardcoded in evaluate.py or made configurable if needed.
    return parser